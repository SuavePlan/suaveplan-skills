...
1. **`packages/control-flow/rate-limiter/README.md`** — overview, install, quick-start, module index
2. **`packages/control-flow/rate-limiter/src/**/*.md`** — co-located docs per implementation file (Purpose, Features, Basic + Advanced examples, full API reference, Implementation notes — all packages in genesis ship ≥200 words per module)
3. **`packages/control-flow/rate-limiter/package.json`** `exports` map — every subpath you can import from
